...
describe("admin notifications direct route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    try {
      const request = new Request("http://localhost/api/v1/admin/notifications/direct?cursor=next", { method, headers: { authorization: "Bearer ignored-client-token" } });
      const response = await handler(request);

      expect(response.status).toBe(200);
      const target = calls[1];
      expect(target?.url).toBe("http://gateway/api/v1/admin/notifications/direct?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/admin/notifications/direct", { method: "POST", headers: { authorization: "Bearer client" } });
