...
const routeParams = {
  ...
  coWriterUserId: "coWriterUserId-value",
};
...
describe("projects/[projectId]/co-writers/[coWriterUserId] route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(handlers)("proxies %s with gateway URL and auth header", async (method, handler) => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/projects/projectId-value/co-writers/coWriterUserId-value?cursor=next", {
        method,
      ...
      expect(target?.url).toBe("http://gateway/api/v1/projects/projectId-value/co-writers/coWriterUserId-value?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/projects/projectId-value/co-writers/coWriterUserId-value", {
        method: "DELETE",
        headers: { authorization: "Bearer client-token" }
      });

      const response = await DELETE(request, { params: Promise.resolve(routeParams) });
