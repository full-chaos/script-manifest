...
const routeParams = { competitionId: "competitionId-value" };
...
describe("admin rankings prestige [competitionId] route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    try {
      const request = new Request("http://localhost/api/v1/admin/rankings/prestige/competitionId-value?cursor=next", { method, headers: { authorization: "Bearer ignored-client-token" } });
      const response = await handler(request, { params: Promise.resolve(routeParams) });

      expect(response.status).toBe(200);
      const target = calls[1];
      expect(target?.url).toBe("http://gateway/api/v1/admin/rankings/prestige/competitionId-value?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/admin/rankings/prestige/competitionId-value", { method: "PUT", headers: { authorization: "Bearer client" } });
      const response = await PUT(request, { params: Promise.resolve(routeParams) });
