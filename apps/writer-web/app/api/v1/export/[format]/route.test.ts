...
describe("export/[format] route", () => {
  ...
  it.each(handlers)("proxies %s with gateway URL and auth header", async (method, handler) => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/export/csv?cursor=next", {
        method,
      ...
      expect(target?.url).toBe("http://gateway/api/v1/export/csv");
      expect(getAuthorizationHeader(target?.init)).toBe("Bearer ignored-client-token");
    } finally {
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/export/csv", {
        method: "GET",
