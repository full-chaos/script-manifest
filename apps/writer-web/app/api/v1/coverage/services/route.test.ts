...
describe("coverage services route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    try {
      const request = new Request("http://localhost/api/v1/coverage/services?cursor=next", { method, headers: { authorization: "Bearer ignored-client-token" } });
      const response = await handler(request);

      expect(response.status).toBe(200);
      const target = calls[0];
      expect(target?.url).toBe("http://gateway/api/v1/coverage/services?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/coverage/services", { method: "GET", headers: { authorization: "Bearer client" } });
