...
describe("notifications/unread-count route", () => {
  ...
  it.each(handlers)("proxies %s with gateway URL and auth header", async (method, handler) => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/notifications/unread-count?cursor=next", {
        method,
      ...
      expect(target?.url).toBe("http://gateway/api/v1/notifications/unread-count?cursor=next");
