...
const routeParams = { provider: "provider-value" };
...
describe("auth oauth [provider] start route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    try {
      const request = new Request("http://localhost/api/v1/auth/oauth/provider-value/start?cursor=next", { method, headers: { authorization: "Bearer ignored-client-token" } });
      const response = await handler(request, { params: Promise.resolve(routeParams) });

      expect(response.status).toBe(200);
      const target = calls[0];
      expect(target?.url).toBe("http://gateway/api/v1/auth/oauth/provider-value/start?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/auth/oauth/provider-value/start", { method: "POST", headers: { authorization: "Bearer client" } });
