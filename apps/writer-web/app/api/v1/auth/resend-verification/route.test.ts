...
describe("auth resend-verification route", () => {
  ...
  it.each([
    ...
    try {
      const request = new Request("http://localhost/api/v1/auth/resend-verification?cursor=next", { method, headers: { authorization: "Bearer ignored-client-token" } });
      const response = await handler(request);

      expect(response.status).toBe(200);
      const target = calls[0];
      expect(target?.url).toBe("http://gateway/api/v1/auth/resend-verification?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/auth/resend-verification", { method: "POST", headers: { authorization: "Bearer client" } });
