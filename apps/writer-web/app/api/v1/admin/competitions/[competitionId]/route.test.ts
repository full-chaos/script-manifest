...
describe("admin competitions [competitionId] route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    try {
      const request = new Request("http://localhost/api/v1/admin/competitions/competitionId-value?cursor=next", { method, headers: { authorization: "Bearer ignored-client-token" } });
      const response = await handler(request, { params: Promise.resolve(routeParams) });

      expect(response.status).toBe(200);
      const target = calls[1];
      expect(target?.url).toBe("http://gateway/api/v1/admin/competitions/competitionId-value?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/admin/competitions/competitionId-value", { method: "PUT", headers: { authorization: "Bearer client" } });
