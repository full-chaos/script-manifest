...
const routeParams = { appealId: "appealId-value" };
...
describe("admin rankings appeals [appealId] resolve route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    try {
      const request = new Request("http://localhost/api/v1/admin/rankings/appeals/appealId-value/resolve?cursor=next", { method, headers: { authorization: "Bearer ignored-client-token" } });
      const response = await handler(request, { params: Promise.resolve(routeParams) });

      expect(response.status).toBe(200);
      const target = calls[1];
      expect(target?.url).toBe("http://gateway/api/v1/admin/rankings/appeals/appealId-value/resolve?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/admin/rankings/appeals/appealId-value/resolve", { method: "POST", headers: { authorization: "Bearer client" } });
      const response = await POST(request, { params: Promise.resolve(routeParams) });
