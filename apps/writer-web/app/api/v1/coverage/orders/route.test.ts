...
describe("coverage orders route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    try {
      const request = new Request("http://localhost/api/v1/coverage/orders?cursor=next", { method, headers: { authorization: "Bearer ignored-client-token" } });
      const response = await handler(request);

      expect(response.status).toBe(200);
      const target = calls[0];
      expect(target?.url).toBe("http://gateway/api/v1/coverage/orders?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/coverage/orders", { method: "GET", headers: { authorization: "Bearer client" } });
