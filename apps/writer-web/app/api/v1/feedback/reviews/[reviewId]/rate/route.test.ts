...
describe("feedback/reviews/[reviewId]/rate route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each(handlers)("proxies %s with gateway URL and auth header", async (method, handler) => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/feedback/reviews/reviewId-value/rate?cursor=next", {
        method,
      ...
      expect(target?.url).toBe("http://gateway/api/v1/feedback/reviews/reviewId-value/rate?cursor=next");
  ...
  it("returns upstream 500 responses", async () => {
    ...
    try {
      const request = new Request("http://localhost/api/v1/feedback/reviews/reviewId-value/rate", {
        method: "POST",
